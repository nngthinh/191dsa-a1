...
void LoadData(void*& pData) {
	...
	while (getline(file, input)) {
		...
		createData_StationLine(ss, StationLine);
	...
	while (getline(file, input)) {
		...
		createData_TrackLine(ss, TrackLine);
...
void createData_City(stringstream& ss, TCity& City) {
	string cell;
...
void createData_Line(stringstream& ss, TLine& Line) {
	string cell;
	//cout << "lineStream.str() = " << ss.str() << std::endl;
...
void createData_StationLine(stringstream& ss, TStationLine& StationLine) {
	string cell;
	//cout << "lineStream.str() = " << ss.str() << std::endl;
...
void createData_Station(stringstream& ss, TStation& Station) {
	string cell;
	//cout << "lineStream.str() = " << ss.str() << std::endl;
...
void createData_System(stringstream& ss, TSystem& System) {
	string cell;
	//cout << "lineStream.str() = " << ss.str() << std::endl;
...
void createData_Track(stringstream& ss, TTrack& Track) {
	string cell;
	//cout << "lineStream.str() = " << ss.str() << std::endl;
...
void createData_TrackLine(stringstream& ss, TTrackLine& TrackLine) {
	string cell;
	//cout << "lineStream.str() = " << ss.str() << std::endl;
...
int TDataset::insert_Station_Station(string& csv_description) {
	...
	TStation Station;
	createData_Station(ss_temp, Station);
...
int TDataset::remove_StationLS_Track_StationGeometry_CityID(string& point, int& city_id) {
	...
	for (size_t i = 0; i < getSize; i++) {
		...
		if (pCur->city_id == city_id) {
			...
			if (pos != -1) {
				pCur->geometry.erase(pos, point.length() + (pos + point.length() == pCur->geometry.length() ? 0 : 1));
				return 0;
			}
		}
	}
